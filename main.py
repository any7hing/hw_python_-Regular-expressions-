...
def final_sort_list():
    for i in range(len(result_list)):
        try:
            ...
            result_dict[" ".join(result_list[i][0].split()[0:2])]= ({'last_name':" ".join(result_list[i][0].split()[0:1]),'first_name':" ".join(result_list[i][0].split()[1:2]),'surname':" ".join(result_list[i][0].split()[2:3]),
'organization':result_list[i][1],'position':result_list[i][2],'phone':result_list[i][3],'email':result_list[i][4]})
